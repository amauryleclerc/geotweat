...
function afficherTweet(data) {
    ...
    for (var i in data.statuses) {
        if (data.statuses[i].coordinates != null && data.statuses[i].coordinates.coordinates[0] !== 0) {
            var lon = data.statuses[i].coordinates.coordinates[0];
            var lat = data.statuses[i].coordinates.coordinates[1];
            var text = data.statuses[i].text;
            text = chercherHashtag(text);
            text = chercherURL(text);
            var li = document.createElement("li");
            li.setAttribute("onclick", "javascript: zoom(" + lon + "," + lat + ")");
            li.innerHTML = text;
            nav.appendChild(li);
...
function zoom(lon, lat) {
    map.zoomOut();
    map.setCenter(new OpenLayers.LonLat(lon, lat).transform(projS, projC));
    map.zoomIn();
}
